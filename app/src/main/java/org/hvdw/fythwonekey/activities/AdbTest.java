...
public class AdbTest extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        String ip = sharedprefs.getString(MySettings.ADB_IP_ENTRY, "").replaceAll("\\s+","");

        int port = Integer.parseInt(sharedprefs.getString(MySettings.ADB_PORT_ENTRY, "5555").replaceAll("\\s+",""));
        ...
        AdbThread adbThread =new AdbThread(adb,command);
        Thread thread1 =new Thread(adbThread);
        thread1.start();

        //If the thread is running for more than 2 seconds, we interrupt it, timeout
        try {
            thread1.join(2000);
            if (thread1.isAlive()) {
                thread1.interrupt();
